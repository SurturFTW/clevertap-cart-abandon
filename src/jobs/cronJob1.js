...
class CronJob1 {
  ...
  async execute() {
    ...
    try {
      ...
      const cartAbandonData = await this.fetchLatestCsvData(
        process.env.S3_CART_ABANDON_BUCKET
      );
      ...
      const deltaData = await csvProcessor.processCartAbandonData(
        cartAbandonData,
        chargedEventsData
      );
      ...
      logger.info(
        `Cron Job 1 completed successfully. Processed ${
          deltaData.length
