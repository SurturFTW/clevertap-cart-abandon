...
class CronJob3 {
  ...
  async execute() {
    ...
    try {
      ...
      const productViewData = await this.fetchLatestCsvData(
        process.env.S3_PRODUCT_VIEW_BUCKET
      );
      const chargedEventsData = await this.fetchLatestCsvData(
        process.env.S3_CHARGED_EVENTS_BUCKET
      );
      ...
      logger.info(
        `Processing ${productViewData.length} product views and ${chargedEventsData.length} charged events`
      );

      // Step 2: Process and filter data
      const deltaData = await this.processProductViewData(
        productViewData,
        chargedEventsData
      );
      ...
      if (deltaData && deltaData.length > 0) {
        ...
        logger.info(
          `Most viewed delta data uploaded successfully: ${fileName}`
        );
      } else {
        logger.info(
          "No most viewed delta data to process. Cron Job 3 completed."
        );
      }

      logger.info(
        `Cron Job 3 completed successfully. Processed ${
          deltaData?.length || 0
        } records in ${Date.now() - startTime}ms`
      );
    } catch (error) {
    ...
    try {
      ...
      const deltaData = Object.values(viewCounts)
        .filter((item) => {
          ...
          return (
            item.count >= this.MIN_VIEW_COUNT &&
            !chargedCombinations.has(combination)
          );
        })
        .map((item) => ({
      ...
      logger.info(
        `Found ${deltaData.length} products with ${this.MIN_VIEW_COUNT}+ views`
      );
      return deltaData;
    } catch (error) {
  ...
  async fetchLatestCsvData(bucketName) {
    try {
      ...
      if (matchingFiles.length === 0) {
        logger.warn(
          `No .csv.gz files found for dates (${dates.join(
            ", "
      ...
      logger.info(
        `Total records processed from ${bucketName}: ${allData.length}`
      );
      return allData;
    } catch (error) {
